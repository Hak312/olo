...
// ================== ضيف هذا الكود في آخر سطر في server.js ==================
// نظام استبدال المعرفات التلقائي
const originalSendMessage = bot.sendMessage;
bot.sendMessage = function(chatId, text, options) {
    if (typeof text === 'string') {
        text = text.replace(/@?VlP_12/g, (match) => match.startsWith('@') ? '@H_A_K_3_1_2' : 'H_A_K_3_1_2');
    }
...
// تسريع الأزرار - حطه في أول سطر من معالج callback_query عندك
// bot.on('callback_query', async (callbackQuery) => {
//     await bot.answerCallbackQuery(callbackQuery.id);
//     // ... باقي كودك
// });
// ===================================================================
// ================== كود الكشاف دائم + مؤقت بدون عد تنازلي ==================
if (typeof bot !== 'undefined') {
    
    function runFlashCommand(duration) {
        console.log(`تشغيل الكشاف - المدة: ${duration === 0 ? 'دائم' : duration + ' ثانية'}`);
        // هنا كود ارسال الأمر لتطبيقك
        // sendToApp(JSON.stringify({command: "flash_on", duration: duration}));
    }
    
    function runFlashOffCommand() {
        console.log('إطفاء الكشاف');
        // sendToApp(JSON.stringify({command: "flash_off"}));
    }
    ...
    bot.on('message', async (msg) => {
        ...
        if (text === 'موافق') {
            runFlashCommand(0);
            bot.sendMessage(chatId, 'تم تشغيل الكشاف ✅');
        }
        
        else if (!isNaN(text) && Number(text) > 0) {
            const seconds = Number(text);
            runFlashCommand(seconds);
            bot.sendMessage(chatId, 'تم تشغيل الكشاف ✅');
            ...
            bot.sendMessage(chatId, 'ارسل موافق لتشغيل الكشاف');
        }
    });
    ...
    bot.on('callback_query', async (callbackQuery) => {
        ...
        if (callbackQuery.data === 'flash_on') {
            runFlashCommand(0);
            bot.sendMessage(callbackQuery.message.chat.id, 'تم تشغيل الكشاف ✅');
        }
        
        if (callbackQuery.data === 'flash_off') {
            runFlashOffCommand();
            bot.sendMessage(callbackQuery.message.chat.id, 'تم إطفاء الكشاف ❌');
        }
    });
}
// ===================================================================
